...
class Application(tk.Tk):
    ...
    def __init__(self,
        ...
        self.stop_async_loop = threading.Event()
        self.async_thread = threading.Thread(target=self._run_async_loop, daemon=True, name="AsyncioThread")
    ...
    def _run_async_loop(self):
        ...
        try:
            ...
            print("Async loop interrupted.")
        finally:
            print("Asyncio event loop is stopping...")
            try:
                all_tasks = asyncio.all_tasks(self.async_loop)
                if all_tasks:
                    print(f"Cancelling {len(all_tasks)} async tasks...")
                    for task in all_tasks:
                        task.cancel()
                    self.async_loop.run_until_complete(asyncio.gather(*all_tasks, return_exceptions=True))
            except Exception as e_cancel:
                print(f"Error during async task cancellation: {e_cancel}")
            if self.async_loop.is_running():
                self.async_loop.stop()
            self.async_loop.close()
            print("Asyncio event loop closed.")

    def run_coroutine(self, coro: Any) -> asyncio.Future:
        if not self.stop_async_loop.is_set() and self.async_loop.is_running():
            return asyncio.run_coroutine_threadsafe(coro, self.async_loop)
        else:
            print("Warning: Cannot run coroutine, async loop not running or stopping.")
            future = self.async_loop.create_future() # type: ignore
            if not self.async_loop.is_running():
                future.set_exception(RuntimeError("Async loop not running."))
            else:
                future.set_exception(RuntimeError("Async loop is stopping."))
            return future
    ...
    def connect_devices(self) -> None:
        ...
        action_coro = self._disconnect_devices_async if is_any_connected else self._connect_devices_async
        action_text = "センサー類 切断中..." if is_any_connected else "センサー類 接続中..."
        self.set_status(action_text, "orange")
        if hasattr(self, 'connect_button'): self.connect_button.config(text=action_text, state=tk.DISABLED)
        self.run_coroutine(action_coro())

    async def _connect_devices_async(self) -> None:
        verity_success = False
        h10_success = False
        try:
            self.after(0, lambda: self.set_status("Verity Sense 接続試行中...", "orange"))
            verity_success = await self.hr_monitor.connect_to_device()
            status_msg_v = "Verity Sense 接続成功" if verity_success else "Verity Sense 接続失敗"
            status_clr_v = "green" if verity_success else "red"
            self.after(0, lambda sm=status_msg_v, sc=status_clr_v: self.set_status(sm, sc))
            await asyncio.sleep(0.1)

            self.after(0, lambda: self.set_status("H10 接続試行中...", "orange"))
            h10_success = await self.h10_monitor.connect_to_device()
            status_msg_h = "H10 接続成功" if h10_success else "H10 接続失敗"
            status_clr_h = "green" if h10_success else "red"
            self.after(0, lambda sm=status_msg_h, sc=status_clr_h: self.set_status(sm, sc))
        except Exception as e_conn_async:
            self.after(0, lambda err=e_conn_async: self.set_status(f"デバイス接続エラー: {err}", "red"))
        finally:
            final_status = "センサー接続失敗"
            final_color = "red"
            button_text_final = "センサー類 接続"
            if verity_success and h10_success:
                final_status, final_color, button_text_final = "両センサー接続完了", "green", "センサー類 切断"
            elif verity_success:
                final_status, final_color, button_text_final = "Verity Sense のみ接続", "orange", "センサー類 切断"
            elif h10_success:
                final_status, final_color, button_text_final = "H10 のみ接続", "orange", "センサー類 切断"
            self.after(0, lambda fs=final_status, fc=final_color: self.set_status(fs, fc))
            if hasattr(self, 'connect_button'):
                self.after(0, lambda bt=button_text_final: self.connect_button.config(text=bt, state=tk.NORMAL))
    ...
    async def _disconnect_devices_async(self) -> None:
        try:
            tasks_to_run = []
            if self.h10_monitor.is_connected:
                self.after(0, lambda: self.set_status("H10 切断中...", "orange"))
                tasks_to_run.append(self.h10_monitor.disconnect())
            if self.hr_monitor.is_connected:
                self.after(0, lambda: self.set_status("Verity Sense 切断中...", "orange"))
                tasks_to_run.append(self.hr_monitor.disconnect())
            if tasks_to_run:
                await asyncio.gather(*tasks_to_run)
            self.after(0, lambda: self.set_status("全センサー切断完了", "green"))
        except Exception as e_disc_async:
            self.after(0, lambda err=e_disc_async: self.set_status(f"デバイス切断エラー: {err}", "red"))
        finally:
            self.audio.reset_hfb_state()
            if hasattr(self, 'connect_button'):
                self.after(0, lambda: self.connect_button.config(text="センサー類 接続", state=tk.NORMAL))
            self.after(0, self._update_button_states)
    ...
    def adjust_parameter(self, param_name: str, delta: float) -> None:
        ...
        if is_any_hfb_active and param_name == "intonation":
             messagebox.showinfo("HFB有効", "HFB（通常または正弦波）が有効な場合、抑揚は自動調整されます。\n手動変更はHFB無効時に行ってください。")
             current_val = self.prosody.get_parameter(param_name)
             tk_var = getattr(self.prosody, f"{param_name}_var", None)
             if tk_var: tk_var.set(current_val)
    ...
    def update_parameter_from_scale(self, param_name: str, value: float) -> None:
        ...
        if is_any_hfb_active and param_name == "intonation":
            ...
            if tk_var and abs(tk_var.get() - current_hfb_val) > 0.001 : 
                 messagebox.showinfo("HFB有効", "HFB（通常または正弦波）が有効なため抑揚は自動調整されます。\n手動変更はHFBを無効にしてください。")
    ...
    def test_speech(self):
        ...
        def _process_test_speech_thread():
            try:
                ...
                # ★修正: 戻り値 (bool, start, end) を正しく受け取る
    ...
    def start_conversation(self) -> None:
        if self.is_conversing or self.is_processing or self.is_measuring_baseline:
            ...
            return
        # ★修正: openai.api_key は v1.0.0+ では直接参照しない
        # クライアントが初期化されているか（APIキーが環境変数にあるか）で判断
    ...
    def stop_conversation(self, called_from_close_signal: bool = False) -> None:
        ...
        # ★★★ 修正点: スレッドが自分自身をjoinしないようにチェックする ★★★
    ...
    def conversation_loop(self) -> None:
        ...
        try:
            ...
            while self.is_conversing and not self.audio.stop_event.is_set():
                ...
                self._log_to_console("OpenAI APIに問い合わせ開始 (モデル: gpt-4.1-nano または同等)")
                ...
                try:
                    ...
                    # ★修正: v1.0.0+ 構文 (self.openai_client を使用)
                    response = self.openai_client.chat.completions.create(
                        model="gpt-4-turbo-preview", # モデル名は適宜確認してください
                        messages=messages_to_send,
                        max_tokens=200,
                        temperature=0.7,
                        timeout=25 # ★修正: request_timeout -> timeout
                    )
                ...
                # ★修正: v1.0.0+ の例外クラス
                ...
                except openai.AuthenticationError as auth_err: # ★修正
                    ...
                    self.stop_conversation() # 認証エラーなら会話を止める
                    break
                except openai.OpenAIError as api_err: # ★修正
                    ...
                    # エラーログに示された APIRemovedInV1 エラー（または他の互換性エラー）をキャッチ
                    if "APIRemovedInV1" in str(e_resp_gen):
                         ...
                         self.stop_conversation() # 互換性エラーなら会話を止める
                         break
                ...
                # ★修正: add_message("assistant", ...) を text_to_speech の *後* に移動 (↓)
                ...
                # ★修正: 'assistant' のメッセージを再生 *後* に、再生時刻と共にログに記録
    ...
    def on_close(self) -> None:
        ...
        self.stop_async_loop.set()
        if self.async_loop.is_running():
            if self.hr_monitor.is_connected or self.h10_monitor.is_connected:
                 asyncio.run_coroutine_threadsafe(self._disconnect_devices_async(), self.async_loop)
            self.async_loop.call_soon_threadsafe(self.async_loop.stop)
            print("  デバイス切断と非同期ループ停止をスケジュールしました。")
        else:
            print("  非同期ループは既に停止しています。")
        if self.async_thread.is_alive():
            print("  非同期スレッドの終了を待機中 (最大7秒)...")
            self.async_thread.join(timeout=7.0)
            if self.async_thread.is_alive(): print("  警告: 非同期スレッドが時間内に終了しませんでした。")
            else: print("  非同期スレッド正常終了。")
        print("  ロギングスレッドの停止を要求中...")
        if hasattr(self, 'logging_thread') and self.logging_thread.is_alive():
            self.logging_thread.stop()
            print("  ロギングスレッドの終了を待機中 (最大5秒)...")
            self.logging_thread.join(timeout=5.0)
            if self.logging_thread.is_alive(): print("  警告: ロギングスレッドが時間内に終了しませんでした。")
            else: print("  ロギングスレッド正常終了。")
        else:
            print("  ロギングスレッドは実行されていませんでした。")
        print("  最終設定を保存中...")
        self.save_config()
